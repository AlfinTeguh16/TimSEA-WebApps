        ...
        <section id="task" class="mt-5">
            <div class="flex flex-col md:flex-row justify-between">
                ...
                <div  class=" flex justify-end gap-4">
                    ...
                    <div>
                        <button type="button" onclick="openModalAddTask()" class="py-2.5 px-4 bg-primary-500 hover:bg-primary-600 text-white rounded-lg">
                            Add Task
                        </button>
                    </div>
                </div>
...
<section id="task-modal" class="fixed inset-0 hidden items-center justify-center z-50 bg-black bg-opacity-50">
    <div class="relative bg-white rounded-lg shadow-lg max-w-2xl w-full h-fit overflow-auto p-4">
        <div class="flex justify-between items-center border-b-2 pb-4 mb-4">
            <h1 class="font-bold text-lg">Add Task</h1>
            <button onclick="closeModal('task-modal')" class="text-gray-600 hover:text-gray-900 text-2xl">&times;</button>
        </div>
        
        <div>
            <form action="{{ route('admin.super-team.tasks.store', $project->id) }}" method="post">
                @csrf
                <div class="flex flex-row justify-between gap-2 text-neutral-600">
                    <div class="w-full">
                        <label for="task" class="text-neutral-600">Task</label>
                        <input type="text" id="task" name="task" placeholder="Add New Task"
                               class="w-full border border-neutral-400 px-2 py-3 rounded-md bg-neutral-100" required>
                    </div>
                    <div class="w-full">
                        <label for="deadline" class="text-neutral-600">Deadline</label>
                        <input type="date" name="deadline" id="deadline"
                               class="w-full border border-neutral-400 px-2 py-[11px] rounded-md bg-neutral-100" required>
                    </div>
                </div>
            
                <div class="mt-2 flex justify-end">
                    <button type="submit" class="w-full md:w-auto items-center justify-center gap-2 py-3 px-5 bg-primary-500 text-white rounded-md">
                        Create Task
                    </button>
                </div>
            </form>
...
function openModalAddTask() {
    ...
    if (modal) {
        ...
        console.error("Modal 'task-modal' not found.");
    }
}
